...
describe('test CheckPermissions', () => {
  it('Correct string permission authentication', () => {
    expect(checkPermissions('base', 'base', target, error)).toEqual('ok');
  });
  it('Correct string permission authentication', () => {
    expect(checkPermissions('base', 'NULL', target, error)).toEqual('error');
  });
  ...
  it('Wrong string permission authentication', () => {
    expect(checkPermissions('admin', 'base', target, error)).toEqual('error');
  });
  it('Correct Array permission authentication', () => {
    expect(checkPermissions(['base', 'admin'], 'base', target, error)).toEqual('ok');
  });
  it('Wrong Array permission authentication,currentAuthority error', () => {
    expect(checkPermissions(['base', 'admin'], 'base,admin', target, error)).toEqual('error');
  });
  it('Wrong Array permission authentication', () => {
    expect(checkPermissions(['base', 'admin'], 'guest', target, error)).toEqual('error');
  });
  ...
  it('authority is string, currentAuthority is array, return ok', () => {
    expect(checkPermissions('base', ['base', 'admin'], target, error)).toEqual('ok');
  });
  it('authority is array, currentAuthority is array, return ok', () => {
    expect(checkPermissions(['base', 'admin'], ['base', 'admin'], target, error)).toEqual('ok');
  });
  it('Wrong Function permission authentication', () => {
    expect(checkPermissions(() => false, ['base'], target, error)).toEqual('error');
  });
  it('Correct Function permission authentication', () => {
    expect(checkPermissions(() => true, ['base'], target, error)).toEqual('ok');
  });
  it('authority is undefined , return ok', () => {
    expect(checkPermissions(null, ['base'], target, error)).toEqual('ok');
  });
});
